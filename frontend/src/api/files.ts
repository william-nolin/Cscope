...
export async function fileHistoryByDate(repositoryId: number): Promise<FileHistoryCommit[]> {
  ...
  categoryToEvolutionType.set("create", TypeFileCommitEvolution.ADD_SOURCE_FILE);
  categoryToEvolutionType.set("delete", TypeFileCommitEvolution.DELETE_SOURCE_FILE);
  categoryToEvolutionType.set("modify", TypeFileCommitEvolution.SET_SOURCE_FILE);
  ...
  const data = (response.data || []).map((point: any) => {
    ...
    return {
      ...
      fileName: filepath,
      typeEvolution: categoryToEvolutionType.get(category) || TypeFileCommitEvolution.SET_SOURCE_FILE,
      Date: new Date(date),
    }
  })
