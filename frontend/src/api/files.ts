...
export async function getGroupedFileTypes ( repositoryId: number ): Promise<CategorizedFileTypes> {
  const endpoint = `/repositories/${repositoryId}/files/stats/grouped_file_types`
  const response = await axios.get( endpoint )

  // The response.data is the Ruby Hash-like structure
